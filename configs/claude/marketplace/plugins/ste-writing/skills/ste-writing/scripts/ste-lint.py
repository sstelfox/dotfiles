...
HARD = ["long_sentence(>20w)", "semicolon", "contraction"]

def lint(text, mode="strict"):
    raw = text
    ...
    counted = {k: x for k, x in v.items() if not (mode == "flavored" and k == "banned_word")}
    total = sum(counted.values())
    hard_total = sum(v[k] for k in HARD) + em
    return {
        "mode": mode,
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Count ASD-STE100 violations in prose.")
    ap.add_argument("files", nargs="*", help="files to lint, reads stdin when omitted")
    ap.add_argument("--json", action="store_true", help="full report for file arguments too")
    ap.add_argument("--mode", choices=["strict", "flavored"], default="strict",
        help="flavored drops banned_word from total")
    args = ap.parse_args()

    if not args.files:
        r = lint(sys.stdin.read(), args.mode)
        print(json.dumps(r, indent=2))
        sys.exit(1 if r["hard_total"] else 0)

    exp = []
    for f in args.files: exp += sorted(glob.glob(f)) if any(c in f for c in "*?[") else [f]
    failed = 0
    for f in exp:
        with open(f) as fh: r = lint(fh.read(), args.mode)
        failed += 1 if r["hard_total"] else 0
        if args.json:
            print(json.dumps({"file": f, **r}, indent=2))
        else:
            print(f"{os.path.basename(f):32} words={r['words']:4d} total={r['total']:3d} per100w={r['total_per100w']:6.2f} em_dash={r['em_dash(slop-marker)']:2d} hard={r['hard_total']:2d}")
